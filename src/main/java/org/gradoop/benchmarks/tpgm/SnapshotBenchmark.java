...
public class SnapshotBenchmark extends AbstractBenchmark {
	 ...
	 */
	private static final String OPTION_QUERY_FROM = "qf";
	/**
	 * Option to declare query to timestamp
	 */
	private static final String OPTION_QUERY_TO = "qt";
	/**
	 ...
	 * Example:
	 * {@code /path/to/flink run -c org.gradoop.benchmarks.tpgm.SnapshotBenchmark
	...
	private static void performSanityCheck(CommandLine cmd) {
		...
		case TYPE_VALID_DURING:
			if (cmd.getOptionValue(OPTION_QUERY_TO) == null) {
				throw new IllegalArgumentException("The used query type needs the parameter '" + OPTION_QUERY_TO
						+ "' to define the query to timestamp.");
			}
			// fall through
		case TYPE_AS_OF:
			if (cmd.getOptionValue(OPTION_QUERY_FROM) == null) {
				throw new IllegalArgumentException("The used query type needs the parameter '" + OPTION_QUERY_FROM
						+ "' to define the query from timestamp.");
			}
			break;
		default:
			break;
	...
	private static void writeCSV(ExecutionEnvironment env) throws IOException {
		String head = String.format("%s|%s|%s|%s|%s|%s|%s|%s", "Parallelism", "dataset", "query-type", "from(ms)",
				"to(ms)", "verify", "count-only", "Runtime(s)");

		String tail = String.format("%s|%s|%s|%s|%s|%s|%s|%s", env.getParallelism(), INPUT_PATH, QUERY_TYPE, QUERY_FROM,
				QUERY_TO, VERIFICATION, COUNT_RESULT, env.getLastJobExecutionResult().getNetRuntime(TimeUnit.SECONDS));
